...
void setup() {
  ...
  Serial.println();
  pinMode(Red_Bike, OUTPUT);
  pinMode(Red_Car, OUTPUT);
  pinMode(Green_Bike, OUTPUT); 
...
void loop() { 
  ...
  digitalWrite(Red_Bike, LOW);
  digitalWrite(Green_Bike, HIGH);
  } else if (result.equals("Other")) {
  digitalWrite(Red_Car, LOW);
  digitalWrite(Green_Car, HIGH);
  digitalWrite(Red_Bike, HIGH);
  digitalWrite(Green_Bike, LOW);  
  }
}
